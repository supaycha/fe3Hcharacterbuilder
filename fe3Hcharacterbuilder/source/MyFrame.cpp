...
MyFrame::MyFrame(wxWindowID id, const wxString& title) : wxFrame(NULL, id, title, wxDefaultPosition, wxDefaultSize, wxDEFAULT_FRAME_STYLE | wxMAXIMIZE) 
{
	...
	avp = new AvailablePanel(weaponmap, battalionmap, equipmap, skilllevelabilitymap, classmasteryabilitymap, this, (int)ID_SINGLE_CONTROL::ID_AVP);
	...
	Bind(wxEVT_COMBOBOX, &MyFrame::BounceRepeatedDDCLSelection_classinnatecheck, this, (int)ID_MISC::ID_DDCL3);
...
void MyFrame::BounceRepeatedDDCHSelection_exclusivitycheck(wxCommandEvent& repititionfromMT) {
	...
	avp->ReceiveDDCHSelection(charactername);
	avp->ReceiveEquipmentExclusivity(charactername);	
...
void MyFrame::BounceRepeatedDDCLSelection_classinnatecheck(wxCommandEvent& repititionfromMT) {
	...
	avp->ReceiveClassMasteryExclusivity(classname);	
